...
build-nginx-proxy-test-debian-dev:
	docker build --build-arg DOCKER_GEN_VERSION=master -t nginxproxy/nginx-proxy:test .

build-nginx-proxy-test-alpine-dev:
	docker build -f Dockerfile.alpine --build-arg DOCKER_GEN_VERSION=master -t nginxproxy/nginx-proxy:test .
